...
# 样列场景
 - 提供一个大公司的ip绑定的界面。一个大公司根据分公司划分不同的网段，每个mac地址绑定在它们所属的网段内并且不能冲突。
 每个mac地址上可以部署多台服务器，动态表单中为服务器内容。
 - 那么首先提供一个可以浏览mac, ip, 服务器的页面。上方一个添加按钮，每个已经绑定的项目都可以编辑。
 - 无论点击编辑还是新建弹出一个Modal，里面可以编辑mac, ip, 服务器ID
